...
Future buildSubscriber(RTCVideoRenderer localRenderer) async {
  ...
  var directorSubscriberOptions = DirectorSubscriberOptions(
      streamAccountId: Constants.accountId, streamName: Constants.streamName);

  /// Define callback for generate new token
  Future<MillicastDirectorResponse> tokenGenerator() =>
      Director.getSubscriber(directorSubscriberOptions);
  ...
  view.on(webRTCEvents['track'], view, (ev, context) {
    var track = ev.eventData as RTCTrackEvent;
  ...
  view.on(SignalingEvents.broadcastEvent, view, (event, context) {
    var eventData = json.encode(event.eventData);
