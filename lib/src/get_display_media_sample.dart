...
import 'package:flutter_webrtc_example/src/dollbyToApi.dart';
...
class _GetDisplayMediaSampleState extends State<GetDisplayMediaSample> {
  ...
  Future<void> _makeCall(DesktopCapturerSource? source) async {
    ...
    try {
      ...
      MillicastService().startStreaming(stream);
      //   await MillicastStreamer().startStreaming(stream);
    } catch (e) {
      print(e.toString());
    }
    if (!mounted) return;
...
//      await MillicastStreamer().stopStreaming();
    } catch (e) {
      print(e.toString());
    }
  }
                ...
                children: <Widget>[
                  ...
                  if (_isPaused)
                    Container(
                      ...
                      decoration: BoxDecoration(color: Colors.black54),
                      child: Icon(Icons.pause, size: 80, color: Colors.white),
                    ),
                ],
              ),
            ),
